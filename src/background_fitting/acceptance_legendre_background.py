...
class acceptance_legendre_2:
    def __init__(self, filtered_data, q2_bins, all_fits,order_costhetal=8, order_costhetak=8, order_phi=8):
        """
    ...
    def fit_SM_observables(self, data, bin_num):
        ...
        def PDF(F_L, A_FB, S3, S4, S5, S7, S8, S9, A_b):
            ...
            func_vals += A_b*background_PDF #A_b is background coefficient 
            return func_vals
        
        def PDF_NLL(F_L, A_FB, S3, S4, S5, S7, S8, S9, A_b):
            func_vals = PDF(F_L, A_FB, S3, S4, S5, S7, S8, S9, A_b)
            return -1. * np.sum(np.log(func_vals))
        
        PDF_NLL.errordef = Minuit.LIKELIHOOD
        minimizer = Minuit(PDF_NLL, F_L=0.3, A_FB=0., S3=0., S4=0., S5=0., S7=0., S8=0., S9=0., A_b=0.2)
